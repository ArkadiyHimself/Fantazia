...
public class LayeredBarrierLayer {
    ...
    public static class LayerBarrier<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T,M> {
        ...
        public void render(@NotNull PoseStack pPoseStack, @NotNull MultiBufferSource pBuffer, int pPackedLight, @NotNull T pLivingEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTick, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
            ...
            LayeredBarrierEffect layeredBarrierEffect = EffectGetter.takeEffectHolder(pLivingEntity, LayeredBarrierEffect.class);
    ...
    private static void renderItem(PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, AbstractClientPlayer playerIn, ModelPart rendererArmWearIn, PlayerModel<AbstractClientPlayer> model, float pPartialTick) {
        ...
        float pV = f * 0.01F % 1.0F;
        VertexConsumer pBufferBuffer = bufferIn.getBuffer(RenderType.create("barrier", DefaultVertexFormat.NEW_ENTITY, VertexFormat.Mode.QUADS, 256, false, true, RenderType.CompositeState.builder().setShaderState(RENDERTYPE_ENERGY_SWIRL_SHADER).setTextureState(new RenderStateShard.TextureStateShard(BARRIER_LAYER, false, false)).setTexturingState(new RenderStateShard.OffsetTexturingStateShard(pU, pV)).setTransparencyState(TRANSLUCENT_TRANSPARENCY).setCullState(NO_CULL).setLightmapState(LIGHTMAP).setOverlayState(OVERLAY).createCompositeState(false)));
