...
public class AbsoluteBarrier {
    ...
    public static class LayerBarrier<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T,M> {
        ...
        public void render(@NotNull PoseStack pPoseStack, @NotNull MultiBufferSource pBuffer, int pPackedLight, @NotNull T pLivingEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTick, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
            ...
            AbsoluteBarrierEffect absoluteBarrierEffect = EffectGetter.takeEffectHolder(pLivingEntity, AbsoluteBarrierEffect.class);
            if (absoluteBarrierEffect == null || !absoluteBarrierEffect.hasBarrier()) return;
            ...
            entityModel.renderToBuffer(pPoseStack, pBufferBuffer, pPackedLight, OverlayTexture.NO_OVERLAY, 1F, 0.6F, furious ? 0.6f : 1f,0.5F);
            VertexConsumer BGvertex = pBuffer.getBuffer(RenderType.entityTranslucent(furious ? BARRIER_BG_FURY : BARRIER_BG));
            entityModel.renderToBuffer(pPoseStack, BGvertex, pPackedLight, OverlayTexture.NO_OVERLAY, 1F, 0.4F, furious ? 0.6f : 1f, 0.125F);
        }
