...
public class EnchantmentCategoryAdvancements {
    ...
    public static void generate(HolderLookup.@NotNull Provider provider, @NotNull Consumer<AdvancementHolder> consumer, @NotNull ExistingFileHelper existingFileHelper) {
        Advancement.Builder.advancement().addCriterion(BuiltInRegistries.ITEM.getKey(Items.ENCHANTING_TABLE).getPath(), InventoryChangeTrigger.TriggerInstance.hasItems(Items.ENCHANTING_TABLE)).save(consumer, Fantazia.location("the_worldliness/enchantments/generic").toString());
    }
}
