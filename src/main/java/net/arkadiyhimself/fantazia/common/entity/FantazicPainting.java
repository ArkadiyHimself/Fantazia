...
public class FantazicPainting extends Painting {
    ...
    public FantazicPainting(Level level, BlockPos pos, Direction direction) {
        ...
        for (Holder<PaintingVariant> variant : registryAccess().registryOrThrow(Registries.PAINTING_VARIANT).getOrCreateTag(FTZPaintingVariantTags.FANTAZIC_PLACAEBLE)) {
            setVariant(variant);
