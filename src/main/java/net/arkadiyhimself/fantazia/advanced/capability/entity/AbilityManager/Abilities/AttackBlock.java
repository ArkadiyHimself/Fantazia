...
public class AttackBlock extends AbilityHolder implements ITicking, ITalentRequire {
    ...
    public AttackBlock(Player player) {
        super(player);
        this.PARRY = new DamageSource(player.level().registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypeRegistry.PARRY), player);
    }
    ...
    public void onHit(LivingAttackEvent event) {
        if ((event.getSource().is(DamageTypes.MOB_ATTACK) || event.getSource().is(DamageTypes.PLAYER_ATTACK))
            ...
            boolean blocked = WhereMagicHappens.Abilities.facesAttack(serverPlayer, event.getSource());
