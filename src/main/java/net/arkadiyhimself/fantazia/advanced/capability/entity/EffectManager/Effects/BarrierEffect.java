        ...
        if (newHP > 0) {
            ...
            if (hitCD <= 0) {
                ...
                for (int i = 1; i <= num; i++) WhereMagicHappens.Abilities.randomParticleOnModel
                        (getOwner(), ParticleRegistry.BARRIER_PIECES.get((new Random()).nextInt(0, ParticleRegistry.BARRIER_PIECES.size()))
            ...
            if (event.getEntity().hasEffect(MobEffectRegistry.BARRIER.get())) EffectCleansing.forceCleanse(event.getEntity(), MobEffectRegistry.BARRIER.get());
            ...
            for (int i = 1; i <= num; i++) {
                WhereMagicHappens.Abilities.randomParticleOnModel(getOwner(),
                        ParticleRegistry.BARRIER_PIECES.get((new Random()).nextInt(0, ParticleRegistry.BARRIER_PIECES.size())).get(),
