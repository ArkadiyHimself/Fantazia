...
public class BarrierLayer {
    public static final ResourceLocation BARRIER_LAYER = new ResourceLocation(CombatImprovement.MODID, "textures/entity_layers/barrier/barrier_armor.png");
    public static final ResourceLocation BARRIER_BG = new ResourceLocation(CombatImprovement.MODID, "textures/entity_layers/barrier/barrier_bg.png");
    ...
    public static class LayerBarrier<T extends LivingEntity, M extends EntityModel<T>> extends RenderLayer<T,M> {
        ...
        public void render(PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, T pLivingEntity, float pLimbSwing, float pLimbSwingAmount, float pPartialTick, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
            BarrierEffect.get(pLivingEntity).ifPresent(barrier -> {
                if (barrier.hasBarrier()) {
                    ...
                    entityModel.renderToBuffer(pPoseStack, pBufferBuffer, pPackedLight, OverlayTexture.NO_OVERLAY, 0F, 0.6F, barrier.barrierColor < 1F ? 0.6F : 1F,0.1019999F + (1 - barrier.barrierColor) * 0.6F);
                    VertexConsumer BGvertex = pBuffer.getBuffer(RenderType.entityTranslucent(BARRIER_BG));
                    entityModel.renderToBuffer(pPoseStack, BGvertex, pPackedLight, OverlayTexture.NO_OVERLAY, barrier.barrierColor, 1F, 1F, 0.05F);
                }
            });
        }
    ...
    private static void renderItem(PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, AbstractClientPlayer playerIn, ModelPart rendererArmwearIn, PlayerModel<AbstractClientPlayer> model, float pPartialTick) {
        ...
        float pV = f * 0.01F % 1.0F;
        float color = BarrierEffect.getUnwrap(playerIn).barrierColor;
